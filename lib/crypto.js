...
function createKey (username, password, opts, cb) {
  ...
  sodium.crypto_generichash(salt, bint.fromString(username), bint.fromString('backpack_salt_001'))

  const key = sodium.sodium_malloc(32)
  sodium.crypto_pwhash_async(key, password, salt, opslimit, memlimit, alg, function (err) {
